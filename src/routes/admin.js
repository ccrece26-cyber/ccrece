  ...
  try {
    ...
    let sql = `
      SELECT pg.id, pg.prestamo_id,
             ...
             c.id AS cliente_id, c.cobrador_id AS cliente_cobrador_id,
    ...
    if (cobrador_id) {
      sql += ' AND (pg.cobrador_id = ? OR pg.operador_id = ?)';
      params.push(cobrador_id, cobrador_id);
    }
  ...
  try {
    ...
    let sqlPagos = `
      SELECT pg.id, pg.prestamo_id,
             pg.cobrador_id,
             pg.cobrador_id AS cobro_registrado_por_id,
             c.cobrador_id AS cliente_cobrador_id,
             pg.monto_pagado, pg.fecha_pago,
    ...
    if (cobrador_id) {
      sqlPagos += ' AND (pg.cobrador_id = ? OR pg.operador_id = ?)';
      paramsPagos.push(cobrador_id, cobrador_id);
    }
