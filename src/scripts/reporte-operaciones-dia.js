...
const { hoyISO, toFechaISO, ZONA_NICARAGUA } = require('../utils/zonaHoraria');
const {
...
function drawSummaryCards(doc, resumen, startY) {
  ...
  const cards = [
    { label: 'Pagos', value: `${resumen.pagos_tx} tx`, sub: money(resumen.pagos_monto), note: `${resumen.pagos_renovacion_tx} renov.` },
    { label: 'Renovaciones', value: String(resumen.renovaciones), sub: 'operaciones', note: '' },
    { label: 'Desembolsos', value: `${resumen.desembolsos_nuevos}+${resumen.desembolsos_renovacion}`, sub: money(resumen.monto_desembolsos), note: 'nuevos + renov.' },
    { label: 'Cierres', value: `${resumen.cierres_tx} tx`, sub: money(resumen.cierres_monto), note: 'cuadre caja' },
  ];
  let x = margin;
  cards.forEach((card) => {
...
function addPageFooters(doc, fecha) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    absoluteText(doc, `Credi Crece · Reporte operaciones ${fecha} · Hora Nicaragua (UTC−6)`, 36, fy, { width: 500 });
    absoluteText(doc, `Pág. ${i - range.start + 1} / ${range.count}`, doc.page.width - 80, fy, { width: 44, align: 'right' });
  }
}
...
function generarPDF(datos) {
  ...
  const OUT = path.join(__dirname, '../../../', outName);
  const doc = new PDFDocument({ size: 'LETTER', margin: 36, layout: 'landscape', bufferPages: true });
